...
st.set_page_config(page_title="Synthetic Angle Gather", layout="wide")

st.markdown("""
<style>
div.block-container { padding-top: 1rem; }
[data-testid="stVerticalBlock"] { gap: 0rem; }
div[data-testid="stNumberInput"] { margin-bottom: -12px; }
...
st.write("Flat AVO angle gather: lithology-based layer model")

def ricker_wavelet(freq, dt, length=0.2):
    t = np.arange(-length / 2, length / 2, dt)
    w = (1 - 2 * np.pi**2 * freq**2 * t**2) * np.exp(-np.pi**2 * freq**2 * t**2)
    return t, w
...
def plot_wiggles(ax, gather, time, angles, angle_inc, scale):
    for j, angle in enumerate(angles):
        ...
        ax.fill_betweenx(time, angle, x, where=(x > angle), color="black", alpha=1.0)
...
for lith in lith_names:
    ...
    vp = c1.number_input("", value=default_props[lith][0], step=50.0, key=f"{lith}_vp")
    vs = c2.number_input("", value=default_props[lith][1], step=50.0, key=f"{lith}_vs")
    rho = c3.number_input("", value=default_props[lith][2], step=0.05, key=f"{lith}_rho")

    lithology[lith] = {"Vp": vp, "Vs": vs, "Density": rho}

st.subheader("Layer Model")

n_layers = st.number_input("Number of Layers", min_value=2, max_value=40, value=3, step=1)
...
for i in range(n_layers):
    ...
    thickness = c1.number_input("", value=100.0, step=5.0, key=f"thick_{i}")
...
df = pd.DataFrame(layers, columns=["Thickness", "Lithology", "Vp", "Vs", "Density"])
...
vp_twt, depth_log, vp_log = build_twt_log(thicknesses, velocities, df["Vp"].values)
vs_twt, _, vs_log = build_twt_log(thicknesses, velocities, df["Vs"].values)
rho_twt, _, rho_log = build_twt_log(thicknesses, velocities, df["Density"].values)
ai_twt, _, ai_log = build_twt_log(thicknesses, velocities, df["AI"].values)
...
plot_wiggles(axes[4], gather, time, angles, angle_inc, wiggle_scale)
